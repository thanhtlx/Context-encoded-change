...
from data_preprocess.f_gen_data import Data_Preprocessor
        ...
        if os.path.exists(cached_features_file):
            ...
            if not evaluate and local_rank == 0:
                for idx, example in enumerate(self.examples[:1]):
                    print("*** Example ***")
                    print("idx: %s", idx)
                    print("source_ids: {}".format(
                        self.preprocessor.tokenizer.convert_ids_to_tokens(example.gen_input_ids)))
        else:
            error = []
            self.examples = []
            print("Creating features from dataset file at %s", os.path.join(file_path, lang + '_' + prefix +
                                                                                  '.jsonl.gz'))
            data = self.load_jsonl_gz(os.path.join(
                file_path, lang + '_commits_' + prefix + '.jsonl.gz'))
            if not isevaluate:
                data = [x for idx, x in enumerate(
                    data) if idx % world_size == local_rank]
            None_num = 0
            for idx, x in enumerate(data):
                if idx % int(len(data) / 5) == 0:
                    print('rank ' + str(args.local_rank) +
                          ': ' + str(idx) + '/' + str(len(data)))
                gen_input_ids, gen_tgt_ids, gen_type_ids = self.preprocessor.input2Features(
                    x)
                if gen_input_ids == None:
                    None_num += 1
                    continue
                self.examples.append(InputFeatures(
                    gen_input_ids, gen_tgt_ids, gen_type_ids))
            if not isevaluate and local_rank == 0:
                for idx, example in enumerate(self.examples[:5]):
                    print("*** Example ***")
                    print("idx: {}".format(idx))
                    print("language: {}".format(lang))
                    print("gen_ids: {}".format(
                        self.preprocessor.tokenizer.convert_ids_to_tokens(example.gen_input_ids)))
                    print("gen_labels: {}".format(
                        self.preprocessor.tokenizer.convert_ids_to_tokens(example.gen_tgt_ids)))
                    print("gen_type: {}".format(example.gen_type_ids))

            print("  Num examples = %d: %d",
                           local_rank, len(self.examples))
...
def train(args, train_datasets, model, preprocessor):
    ...
    train_samplers = [RandomSampler(train_dataset)
    ...
    if os.path.exists(scheduler_last):
        ...
        scheduler.load_state_dict(torch.load(
            scheduler_last, map_location='cpu'))
    ...
    print("  Num examples = %d", sum([len(train_dataset) for train_dataset in train_datasets]) * (
    ...
    for _, batch in merge_dataset(train_dataloaders, [len(x) for x in train_datasets], 0.7):
        ...
        loss, _, _ = model(gen_input_ids, gen_tgt_ids,
                           gen_type_ids, input_embeds=inp_emb)
        if args.n_gpu > 1:
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            scheduler.step()
            global_step += 1
            avg_loss = round((tr_loss - logging_loss) /
                             (global_step - tr_nb), 6)
            if global_step % 100 == 0:
                print(datetime.datetime.now(),"  steps: %s loss: %s", global_step,
                            round(avg_loss, 6))
            ...
            if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                ...
                if hasattr(model, 'module'):
                    torch.save(model.module.state_dict(),
                               os.path.join(output_dir, 'module.bin'))
                torch.save(model.state_dict(), os.path.join(
                    output_dir, 'whole_model.bin'))
                ...
                torch.save(model.state_dict(), os.path.join(
                    last_output_dir, 'whole_model.bin'))
            ...
            if global_step == args.test_step:
                if hasattr(model, 'module'):
                    torch.save(model.module.state_dict(), os.path.join(
                        args.output_dir, 'module.bin'))
                torch.save(model.state_dict(), os.path.join(
                    args.output_dir, 'whole_model.bin'))
                torch.save(args, os.path.join(
                    args.output_dir, 'training_args.bin'))
                print(
                    "Saving model checkpoint to %s for testing", args.output_dir)
                break
...
def evaluate(args, model, preprocessor, eval_when_training=False):
    ...
    eval_datasets = load_and_cache_examples(
        args, preprocessor, isevaluate=True)
    if not os.path.exists(eval_output_dir) and args.local_rank in [-1, 0]:
        os.makedirs(eval_output_dir)

    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
    # Note that DistributedSampler samples randomly
    eval_samplers = [SequentialSampler(eval_dataset)
    ...
    for eval_dataloader in eval_dataloaders:
        for gen_input_ids, gen_tgt_ids, gen_type_ids in iter(
            ...
            with torch.no_grad():
                _, loss, num = model(
                    gen_input_ids, gen_tgt_ids, gen_type_ids, input_embeds=inp_emb)
    ...
    for eval_dataloader in eval_dataloaders:
        for source_ids, labels, type_ids in iter(eval_dataloader):
            ...
            for label in labels:
                ...
                gold = preprocessor.tokenizer.decode(
                    label, clean_up_tokenization_spaces=False)
                tgtss.append(gold.replace('[MSG]', '').strip())

            with torch.no_grad():
                preds = model(gen_input_ids=source_ids,
                              gen_type_ids=type_ids, input_embeds=inp_emb)
                for pred in preds:
                    t = pred[0].cpu().numpy()
                    t = list(t)
                    if 0 in t:
                        t = t[:t.index(0)]
                    text = preprocessor.tokenizer.decode(
...
def main():
    ...
    parser.add_argument("--beam_size", default=1, type=int,
                        help='beam size when doing inference')
    ...
    parser.add_argument('--mode', type=str,
                        help="For different mode to train.")
    parser.add_argument('--weight', type=float, default=1.0,
                        help="The weight for contrastive loss.")
    parser.add_argument('--queue_size', type=int, default=65536,
                        help="The queue length for negative samples.")
    ...
    parser.add_argument('--ignore_type', type=str, default='',
                        help="Ignore mutation type.")
    parser.add_argument('--test_step', type=int, default=100000,
                        help='which training step to start test')
    parser.add_argument('--not_embed', action='store_true',
                        help='whether to use segment embedding')
    ...
    if args.saved_path is not None:
        args.model_name_or_path = os.path.join(
            args.saved_path, 'pytorch_model.bin')
    ...
    target_size = 150 if args.finetune_task == 'msg' else args.block_size
    data_pre = Data_Preprocessor(
        args.block_size, target_size, tokenizer, model_type=args.model_type, args=args)
    ...
    if args.test_path is not None:
        ...
        train_dataset = load_and_cache_examples(
            args, data_pre, isevaluate=False)
